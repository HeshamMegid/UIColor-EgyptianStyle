//
//  UIColor+EgyptianStyle.m
//
//  Created by Hesham Abd-Elmegid on 27/3/13.
//  Copyright (c) 2013 Hesham Abd-Elmegid. All rights reserved.
//

#import "UIColor+EgyptianStyle.h"

@implementation UIColor (EgyptianStyle)

+ (UIColor *)betenganyColor {
    return [UIColor colorWithRed:38.0f/255.0f green:0.0f/255.0f blue:0.0f/255.0f alpha:1.0f];
}

+ (UIColor *)feranyColor {
    return [UIColor colorWithRed:100.0f/255.0f green:100.0f/255.0f blue:100.0f/255.0f alpha:1.0f];
}

+ (UIColor *)fo7lo2yColor {
    return [UIColor colorWithRed:63.0f/255.0f green:160.0f/255.0f blue:182.0f/255.0f alpha:1.0f];
}

+ (UIColor *)fosdo2yColor {
    return [UIColor colorWithRed:163.0f/255.0f green:197.0f/255.0f blue:63.0f/255.0f alpha:1.0f];
}

+ (UIColor *)gamalyColor {
    return [UIColor colorWithRed:186.0f/255.0f green:148.0f/255.0f blue:102.0f/255.0f alpha:1.0f];
}

+ (UIColor *)kharawyColor {
    return [UIColor colorWithRed:79.0f/255.0f green:53.0f/255.0f blue:26.0f/255.0f alpha:1.0f];
}

+ (UIColor *)lamoonyColor {
    return [UIColor colorWithRed:252.0f/255.0f green:252.0f/255.0f blue:40.0f/255.0f alpha:1.0f];
}

+ (UIColor *)lightKharawyColor {
    return [UIColor colorWithRed:182.0f/255.0f green:148.0f/255.0f blue:96.0f/255.0f alpha:1.0f];
}

+ (UIColor *)samawyColor {
    return [UIColor colorWithRed:1.0f/255.0f green:148.0f/255.0f blue:96.0f/255.0f alpha:1.0f];
}

+ (UIColor *)zatoonyColor {
    return [UIColor colorWithRed:76.0f/255.0f green:81.0f/255.0f blue:41.0f/255.0f alpha:1.0f];
}

@end
